...
\begin{slide}{Chord finger tables}
  \begin{block}{Principle}
    \begin{itemize}
    \item Each node \id{p} maintains a \red{finger table} \mathexpr{\id{FT_p}[]} with at most \mathexpr{m}
      entries:
      \[
      ...
      \mathexpr{2^{i-1}}.
    \item To look up a key \mathexpr{k}, node \id{p} forwards the request to node with index \mathexpr{j}
      satisfying
      \[
      q = \id{FT_p}[j] \leq k < \id{FT_p}[j+1]
      \]
    \item If \mathexpr{p < k < \id{FT_p}[1]}, the request is also forwarded to \mathexpr{\id{FT_p}[1]}
    \end{itemize}
  \end{block}
\end{slide}
...
\begin{slide}{Exploiting network proximity}
  ...
  \begin{block}{Solutions}
    \begin{itemize}
    \item\red{Topology-aware node assignment}: When assigning an ID to a node, make sure that nodes
      close in the ID space are also close in the network. \blue{Can be very difficult}.
    \item\red{Proximity routing}: Maintain more than one possible successor, and forward to the
      ...
      \mathexpr{INT}.
    \item\red{Proximity neighbor selection}: When there is a choice of selecting who your neighbor
...
%     \begin{itemize}\tightlist
%     \item \blue{Global level:} Consists of the high-level directory nodes. Main aspect is that these directory
%       nodes have to be jointly managed by different administrations
%     \item \blue{Administrational level:} Contains mid-level directory nodes that can be grouped in such a way
%       that each group can be assigned to a separate administration.
%     \item \blue{Managerial level:} Consists of low-level directory nodes within a single administration. Main
