...
%------------------------------------------------
\begin{frame}{System Structure and Trusted Computing Base}
  \centering

  \begin{tikzpicture}[node distance=5mm]
    \node[draw, rectangle, rounded corners=2pt,
      minimum width=7cm, minimum height=0.9cm] (apps)
    {Applications};
    ...
    \node[draw, dashed, very thick, fit=(vmm)(hyp), inner sep=8pt] {};
  \end{tikzpicture}

  \vspace{0.6em}
  \small
  \textbf{Figure:} Logical layering of a virtual machine system. The trusted computing
  base consists primarily of the hypervisor core and device model.
\end{frame}

%------------------------------------------------
\begin{frame}{CPU Virtualization: VM Entry and Exit}
  \centering

  \begin{tikzpicture}
    \node[draw, rectangle, rounded corners=2pt,
      minimum width=7cm, minimum height=1cm] (guest)
    at (0,3)
    {Guest Execution (Non-root Mode)};

    \node[draw, rectangle, rounded corners=2pt,
      minimum width=7cm, minimum height=1cm] (hyp)
    at (0,1.2)
    {Hypervisor Execution (Root Mode)};

    \draw[-Latex] (guest.south) -- node[right] {\small VM Exit} (hyp.north);

    \draw[-Latex] (hyp.north east)
    .. controls +(2,1) and +(2,-1) ..
    node[right] {\small VM Entry}
    (guest.south east);

    % IMPORTANT: no itemize inside node; use paragraph text with bullets
    \node[draw, rectangle, rounded corners=2pt,
      text width=7cm, align=left, inner sep=5pt]
    at (0,-0.45)
    {\small \textbf{Examples of VM exit causes:}\\
      \(\bullet\) privileged instruction\\
      \(\bullet\) control register update\\
      \(\bullet\) page fault or device access};
  \end{tikzpicture}

  \vspace{0.6em}
  \small
  \textbf{Figure:} Hardware-supported transitions between guest and hypervisor
  execution contexts.
\end{frame}

%------------------------------------------------
\begin{frame}{Memory Virtualization Using Nested Paging}
  \centering

  \begin{tikzpicture}[node distance=6mm]
    \node[draw, rectangle, rounded corners=2pt, minimum width=8cm] (gva)
    {Guest Virtual Address (GVA)};
    \node[draw, rectangle, rounded corners=2pt, minimum width=8cm,
      below=of gva] (gpt)
    {Guest Page Tables};
    \node[draw, rectangle, rounded corners=2pt, minimum width=8cm,
      below=of gpt] (gpa)
    ...
    \node[draw, dashed, very thick, fit=(gpt)(npt), inner sep=6pt] {};
  \end{tikzpicture}

  \vspace{0.6em}
  \small
  \textbf{Figure:} Two-dimensional address translation used in hardware-assisted
  memory virtualization.
\end{frame}

%------------------------------------------------
\begin{frame}{I/O Virtualization Design Space}
  \centering

  \begin{tikzpicture}
    \draw[line width=0.9pt] (-4,2.2) -- (4,2.2);
    \node at (-4,2.6) {\small Compatibility};
    \node at (4,2.6) {\small Performance};

    \node[draw, rectangle, rounded corners=2pt,
      minimum width=4.5cm] (emu) at (-3,1.4)
    {Device Emulation};

    \node[draw, rectangle, rounded corners=2pt,
      minimum width=4.5cm] (pv) at (0,1.4)
    {Paravirtual I/O};

    \node[draw, rectangle, rounded corners=2pt,
      minimum width=4.5cm] (pas) at (3,1.4)
    {Device Passthrough};

    \draw[-Latex] (emu.north) -- (-3,2.2);
    \draw[-Latex] (pv.north) -- (0,2.2);
    \draw[-Latex] (pas.north) -- (3,2.2);
  \end{tikzpicture}

  \vspace{0.6em}
  \small
  \textbf{Figure:} Spectrum of I/O virtualization techniques and their trade-offs.
\end{frame}
