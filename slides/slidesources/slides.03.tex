...
\begin{slide}{Introduction to threads}
  \begin{block}{Basic idea} 
    ...
    \begin{description}
    \item[\red{Processor}:] Provides a set of instructions along with the capability of automatically
      executing a series of those instructions.
    \item[\red{Thread}:] A minimal software processor in whose \blue{context} a series of instructions can be
    ...
    \item[\red{Process}:] A software processor in whose context one or more threads may be executed. Executing
...
\begin{slide}{Context switching}
  \begin{block}{Contexts}
    \begin{itemize}
    \item<1->\red{Processor context}: The minimal collection of values stored in the registers of a processor
      used for the execution of a series of instructions (e.g., stack pointer, addressing registers, program
      counter).
    \item<2->\red{Thread context}: The minimal collection of values stored in registers and memory, used for
      the execution of a series of instructions (i.e., processor context, state).
    \item<3->\red{Process context}: The minimal collection of values stored in registers and memory, used for
...
\begin{slide}{Threads and operating systems}
  \begin{block}{Kernel solution}
    ...
    \blue{all} operations return as system calls:
    \begin{itemize}
    \item Operations that block a thread are no longer a problem: the \blue{kernel schedules another available
      thread} within the same process.
    \item handling external events is simple: the \blue{kernel} (which catches all events) \blue{schedules the
      thread associated with the event}.
    \item The problem is (or used to be) the \blue{loss of efficiency} because each thread
...
\end{slide}
  ...
  \begin{slide}{User and kernel threads combined}
    \begin{block}{Principle operation}
      ...
      \item<2-> User thread does system call \mathexpr{\Rightarrow} \red{the kernel thread that is
        executing that user thread, blocks}. The user thread remains \blue{bound} to the kernel thread.
      \item<3-> The kernel can \red{schedule another kernel thread having a runnable user thread bound to
